...
use App\Filament\Resources\TransactionOutResource\Pages;
...
class TransactionOutResource extends Resource
{
        ...
        return $table
            ...
            ->filters([
                SelectFilter::make('session_id')
                    ->label('Session ID')
                    ->options(fn() => Transaction::where('type', 'OUT')->whereNotNull('session_id')->where('session_id', '!=', '')->select('session_id')->distinct()->orderBy('session_id')->pluck('session_id', 'session_id')->toArray())
                    ->searchable()
            ...
            ->headerActions([
                Action::make('export_excel')
                    ->label('Export Excel')
                    ->action(fn() => Excel::download(new TransactionExport('OUT'), 'barang-keluar.xlsx')),

                Action::make('export_pdf')
                    ->label('Export PDF')
                    ->url(fn() => url('/export-transactions-pdf/OUT'))
                    ->openUrlInNewTab(),
            ])
            ->actions([
